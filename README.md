# Kodi driver for NEEOInc

This is based on the driver by **Niels de Klerk** [Github](https://github.com/nklerk/)
...
Use your computers console and go to the path where the files are extracted, Edit the file Debug_NEEO_DRIVER-KODI.js to your personal setup, then Start the Debug_NEEO_DRIVER-KODI.js file with node.
...
- "Announce services to other systems", (not possible on kodi v17.x on android, Upgrade to kodi v18 Beta)
...
- Fixed showing list items x times.
...
- Fixed content aware controls being dependent on English language.
- Added disconnected message to list view.
- Added SUBTITLE and AUDIO (Language) Buttons.
- Removed unused code.
- Fixed lists limited to 64 items.
- Added PLAY PAUSE button for handling play pause in Player, is mapped to OK button.
...
- Bugfix: Empty list were causing an issue.
...
- Reworked the way item lists are build. now steps of 64 items are fetched.
- Code cleaning.
- Fixed images.

## Version 1.1.2

- fixed recurring filter issue
...
- Fixed a WOL bug.
- Added PVR TV stations. Shows a list of TV channels and if EPG info is available also the current broadcast title.
- Added PVR Radio stations. Shows a list of Radio channels and if EPG info is available also the current broadcast title.
- Added content aware buttons of Channel UP and Channel Down.
- Added Recent Albums

## Version 1.0.12

- Added support for Music albums and tracks.
...
- Removed Circular dependencies, Thanks to Michael Kohler
...
- Added content aware controls. i.e. using cursor in movie to skip.
