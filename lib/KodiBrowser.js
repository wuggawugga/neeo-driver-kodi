...
module.exports = class KodiBrowser {
	...
	constructor(id, controller, client) {
		...
		this.routes = [];
		this.routes.push({
			regexp: '/music/artist/:artist_id/albums',
			method: (path_in, params) => {
				return new BluePromise((resolve, reject) => {
					this.library.getAlbumsByArtist(path_in, params).then((list) => {
		...
		this.routes.push({
			regexp: '/music/album/:album_id',
			method: (path_in, params) => {
				return new BluePromise((resolve, reject) => {
					this.library.getAlbum(path_in, params).then((list) => {
						resolve(this.formatList(list, params));
					});
				});
			}
...
*/
	...
	router(path, params) {
		...
		for(var route of this.routes) {
			debug('ROUTE' + route);
			regexp = pathToRegexp(route.regexp, keys);
			res = regexp.exec(path);
			debug('REGEX', res, keys);
			if(res !== null) {
				debug('HEYO');
				return route.method(res, params);
			}
		}
		if(res == null) {
			debug('Invalid path');
...
//						debug('response', response);
						...
						for(var item_in of response[key]) {
							...
							// thumbnailUri
							if(typeof item_in.thumbnail !== undefined) item_out.thumbnailUri = this.controller.fetchKodiImage(this.id, item_in.thumbnail, 100);
							// browseIdentifier
							if(node.list.path) {
								item_out.browseIdentifier = node.list.path.replace(/\*/g, item_in[node.list.key]);
		...
		switch(media_item_type) {
			case 'song':
				try {
					...
					if(artist_thumb) o.thumbnailUri = this.controller.fetchKodiImage(this.id, artist_thumb, 100)
					list.addListItem(o);
					...
					if(i.album) {
						...
						if(i.thumbnail) o.thumbnailUri = this.controller.fetchKodiImage(this.id, i.thumbnail, 100)
						list.addListItem(o);
					}
					if(i.lyrics) {
					...
					o = {
						thumbnailUri: this.controller.fetchKodiImage(this.id, i.thumbnail, 215)
					};
				...
				.then((response) => {
					...
					for(var item of response.items) {
						var params_item = {
							title: item.title,
							label: item.label,
							thumbnailUri: this.controller.fetchKodiImage(this.id, item.thumbnail, 100),
			...
			if(media_item_type == 'movie') {
				...
				if(s.playlists[1].size == 1) {
					...
					var params_item = {
						...
						thumbnailUri: this.controller.fetchKodiImage(this.id, s.media.item.thumbnail, 100)
					};
/*
					let conf = {
