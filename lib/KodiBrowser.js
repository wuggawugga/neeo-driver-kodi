...
//						debug('response', response);
						...
						for(var item_in of response[key]) {
							...
							// thumbnailUri
							if(typeof item_in.thumbnail !== undefined) item_out.thumbnailUri = this.client.fetchKodiImage(item_in.thumbnail, 215);
							// browseIdentifier
		...
		switch(media_item_type) {
			case 'song':
				try {
					...
					if(artist_thumb) o.thumbnailUri = this.client.fetchKodiImage(artist_thumb, 215)
					list.addListItem(o);
					...
					if(i.album) {
						...
						if(i.thumbnail) o.thumbnailUri = this.client.fetchKodiImage(i.thumbnail, 215)
						list.addListItem(o);
					}
					if(i.lyrics) {
					...
					o = {
						thumbnailUri: this.client.fetchKodiImage(i.thumbnail, 215)
					};
				...
				.then((response) => {
					...
					for(var item of response.items) {
						var params_item = {
							title: item.title,
							label: item.label,
							thumbnailUri: this.client.fetchKodiImage(item.thumbnail, 215),
			...
			if(media_item_type == 'movie') {
				...
				if(s.playlists[1].size == 1) {
					...
					var params_item = {
						...
						thumbnailUri: this.client.fetchKodiImage(s.media.item.thumbnail, 215)
					};
/*
					let conf = {
