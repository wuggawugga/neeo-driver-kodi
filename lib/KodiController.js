...
//    this._componentUpdateCallbacks = {};
  ...
  registerDevice(adapter_id, device) {
    debug('registerDevice', adapter_id, device.name);
    this.devices[adapter_id] = device;
  }
  ...
  async onButtonPressed(button_id, device_id) {
    debug('onButtonPressed()', button_id, '@', device_id);
    let window = this.clients[device_id].state.get('state.current_window_id');
    ...
    if(method == 'INTERNAL') {
      switch(params.action) {
        case 'WoL':
          return this.clients[device_id].wakeDevice();
          break;
      }
    } else {
...
//        throw e;
        ...
        if(kodiCommands.commands[cmd].event) {
          let event = kodiCommands.commands[cmd].event;
          let event_params = kodiCommands.commands[cmd].event_params || {};
          this.clients[device_id].state.handleEvent(event, event_params);
        }
        return result;
      });
    }
  ...
  sendComponentUpdate(device_id, component_id, component_value) {
    ...
    for(const [id, device] of Object.entries(this.devices)) {
        if(this.devices[id].hasKodiInstance(device_id)) {
          this.devices[id].sendComponentUpdate(device_id, component_id, component_value);
        }
    }
  }
