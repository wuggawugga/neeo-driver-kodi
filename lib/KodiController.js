...
//        this._adapters[this.instances[id].adapterName] = id;
  ...
  async getImageUrl(device_id, image_id) {
    ...
    if(this.clients[device_id].state.media.item.thumbnail) {
      url = await this.clients[device_id].getKodiImage(this.clients[device_id].state.media.item.thumbnail, size);
    }
  ...
  getSensorValue(device_id, sensor_id) {
    ...
    switch(sensor_id) {
      case 'SWITCH_PLAYING':
        ...
        break;
      case 'SWITCH_SHUFFLE':
        let player_id = this.clients[device_id].state.get('state.active_player_id');
        let shuffled = this.clients[device_id].state.get('players.'+player_id+'.shuffled');
