...
/*
                debug('PLAY', argv[2]);
                debug('CLEAR');
                this.clients[device_id].send('Playlist.Clear', {playlistid: 0}).then(() => {
                  debug('ADD', argv[2]);
                  this.clients[device_id].send('Playlist.Add', {item: {songid: argv[2]}}).then(() => {
                    debug('OPEN');
                    this.clients[device_id].send('Player.Open', {item: {playlistid: 0}});
                  });
                });
*/
                break;
