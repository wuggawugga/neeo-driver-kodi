...
class KodiController {
  ...
  onButtonPressed(button_id, device_id) {
    ...
    this.clients[device_id].state.getCurrentWindow().then((window) => {
...
//          }
  ...
  getImageUrl(device_id, image_id) {
    ...
    if(this.clients[device_id].state.media.item.thumbnail) {
      url = this.clients[device_id].fetchKodiImage(this.clients[device_id].state.media.item.thumbnail, size);
    }
