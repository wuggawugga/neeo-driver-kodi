...
*/
  ...
  browseDirectory(device_id, directory_id, params) {
    debug('browseDirectory()', device_id, directory_id, params);
    let list = this.browsers[device_id].browseDirectory(directory_id, params);
    debug('list', list);
    return list;
  }
  ...
  listAction(device_id, directory_id, params) {
    ...
    if(params.actionIdentifier) {
      ...
      switch(method) {
        case 'Player.GoTo':
//          let active_player = this.clients[device_id].state.getState('activePlayer');
          let active_player = parseInt(argv[1]);
          let playlist_pos = parseInt(argv[2]);
          let params = {playerid: active_player, to: playlist_pos};
          this.clients[device_id].send('Player.GoTo', params);
          break;
        case 'KodiBrowser.getIndexItems':
          break;
      }
    }
  }
