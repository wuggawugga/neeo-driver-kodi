...
//		console.log(this.toString());
	...
	updateApplication(props_in) {
		...
		return new BluePromise((resolve, reject) => {
			this.client.send('Application.GetProperties', {properties: props} ).then( reply => {
				...
				this.controller.sendComponentUpdate(this.id, 'VOLUME_SENSOR', this.state.volume);	// Media player widget
				this.controller.sendComponentUpdate(this.id, 'MUTE_SENSOR', this.state.muted);	// Media player widget
				resolve(reply);
			});
		});
	}
	...
	updateGUI() {
		...
		return new BluePromise((resolve, reject) => {
			BluePromise.join(
				this.client.send('XBMC.GetInfoBooleans', { booleans: ['System.HasActiveModalDialog', 'System.HasVisibleModalDialog'] } ).then((result) => {
					this.state.has_menu = result['System.HasActiveModalDialog'];
				}),
				this.client.send('GUI.GetProperties', {properties: ['currentwindow']} ).then( (reply) => {
					this.debugData('updateGUI() DATA', reply);
					if(reply.currentwindow && reply.currentwindow.id) {
						let window_id = reply.currentwindow.id;
						if(window_id != this.state.current_window_id) {
							this.state.previous_window_id = this.state.current_window_id;
							this.state.current_window_id = window_id;
						}
					}
				}),
				(booleans, properties) => {
			...
			try {
				...
				.then((reply) => {
					...
					if(this.state.isPlaying != reply['Player.Playing']) {
						...
						this.client.sendComponentUpdate(this.id, 'PLAYING_SENSOR', this.state.isPlaying);	// Media player widget
					}
	...
	getPlayerProperties(player_id) {
		...
		return new BluePromise((resolve, reject) => {
			if(player_id == undefined) reject('Invalid player ID');
			this.client.send('Player.GetProperties', {playerid: player_id, properties: ['audiostreams', 'currentaudiostream', 'type', 'partymode', 'speed', 'time', 'percentage', 'totaltime', 'playlistid', 'position', 'repeat', 'shuffled', 'subtitleenabled', 'currentsubtitle', 'subtitles', 'live']})
			.then((player) => {
				if(player.shuffled != this.players[player_id].shuffled) {
					let shuffle = player.shuffled == false ? false : true;
					this.client.sendComponentUpdate(this.id, 'SWITCH_SHUFFLE', shuffle);
					this.client.sendComponentUpdate(this.id, 'SHUFFLE_SENSOR', shuffle);	// Media player widget
				}
				if(player.repeat != this.players[player_id].repeat) {
					let repeat = player.repeat == 'off' ? false : true;
					this.client.sendComponentUpdate(this.id, 'SWITCH_REPEAT', repeat);
					this.client.sendComponentUpdate(this.id, 'REPEAT_SENSOR', repeat);	// Media player widget
				}
				Object.assign(this.players[player_id], player);
				resolve(player);
			});
		});
	}
		...
		return new BluePromise((resolve, reject) => {
			...
			if(thumbnail) {
				...
				this.controller.getImageUrl(this.id, 'IMAGE_NOW_PLAYING_THUMBNAIL_LARGE').then((url) => {
					...
					this.client.sendComponentUpdate(this.id, 'COVER_ART_SENSOR', url);	// Media player widget
				});
	...
	updateMediaCaption() {
		...
		if(s != this.media.caption) {
			...
			this.client.sendComponentUpdate(this.id, 'TITLE_SENSOR', s);	// Media player widget
		}
		return s;
	}
	...
	updateMediaDescription() {
		...
		if(s != this.media.description) {
			...
			this.client.sendComponentUpdate(this.id, 'DESCRIPTION_SENSOR', s);	// Media player widget
		}
		return s;
	}
	...
	handleEvent(event, params) {
		...
		switch(event) {
			...
			case 'Player.OnPropertyChanged':
				this.getPlayerProperties(this.state.active_player_id);
				break;
