...
module.exports = class KodiClient {
   ...
   */
  async getKodiImage(internal_url, size_in) {
    debug('getKodiImage()');//, internal_url, size_in);
...
//					let url_out = await this.fetchImage(external_url);
					let url_out = this.fetchImage(external_url);
					debug('BERFORE', url_out);
					url_out = this.getLocalImageUrl(external_url, size);
					debug('ARFTRRR', url_out);
					return url_out;
				}
			} else {
		...
		debug('RETURNNNNNNNNNNNNNNNNNNNNN', url_out);
		return await url_out;
...
//				receptacle.set(hash, {type: 'placeholder', data: null});
	      ...
	      .catch(function (error) {
	        ...
	        reject(error.message);
				...
				debug('PPPPPPPPPPPPPPPPPPPPPPP', typeof p, p.constructor.name);
			}
    });
  }
