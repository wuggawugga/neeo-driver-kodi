			...
			this.library.getAlbumDetails(album_id).then((album) => {
				...
				this.library.getSongs(params_out).then(async (songs) => {
					...
					list_out.addListButtons([
						{ title: 'Play Album', actionIdentifier: 'AudioPlayer/playAlbum/' + album.albumid, uiAction: 'close' },
						{ title: 'Queue Album', actionIdentifier: 'AudioPlayer/queueAlbum/' + album.albumid, uiAction: 'close' }
					]);
