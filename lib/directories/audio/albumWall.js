		...
		return new BluePromise((resolve, reject) => {
			...
			this.library.getAlbums(params_out).then(async (response) => {
				...
				for(let album of response.albums) {
					...
					let url = this.browser.client.getKodiImage(album.thumbnail, 215);
					items.push({thumbnailUri: await url, browseIdentifier: '/audio/album/'+album.albumid});
