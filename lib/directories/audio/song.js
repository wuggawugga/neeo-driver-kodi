		...
		return new BluePromise((resolve, reject) => {
			...
			this.library.getSongDetails(song_id).then((song) => {
				...
				list_out.addListButtons([
					{ title: 'Play Song', uiAction: 'close', actionIdentifier: ['AudioPlayer', 'playSong', song_id].join('|') },
					{ title: 'Queue Song', uiAction: 'goBack', actionIdentifier: ['AudioPlayer', 'queueSong', song_id].join('|') }
				]);
				...
				if(song.albumid) {
					...
					list_out.addListButtons([
						{ title: 'Play Album', uiAction: 'close', actionIdentifier: [ 'AudioPlayer', 'playAlbum', album_id, track ].join('|') },
						{ title: 'Queue Album', uiAction: 'goBack', actionIdentifier: [ 'AudioPlayer', 'queueAlbum', album_id ].join('|') }
					]);
