		...
		return new BluePromise((resolve, reject) => {
			...
			this.library.getAlbumDetails(album_id).then((album) => {
				...
				this.library.getSongs(params_out).then((songs) => {
					...
					if(album.thumbnail) {
						item_out.thumbnailUri = this.browser.client.fetchKodiImage(album.thumbnail, 215);
					}
					list_out.addListItem(item_out);

					list_out.addListItem({
						title: 'Thumbnail (original)',
						// label: uri_kodi,
						// thumbnailUri: uri_kodi,
					});

					if(album.art) {
						for(let [key, value] of Object.entries(album.art)) {
							debug('OBJECT', key, value);
							list_out.addListItem({
								title: 'ART',
								label: key,
								thumbnailUri: this.browser.client.fetchKodiImage(value, 215)
							});
						}
					}
					...
					for(var item_in of songs.songs) {
						var item_out = {
							...
							browseIdentifier: '/music/song/' + item_in.songid,
						};
						if(item_in.duration) {
							let duration = null;
