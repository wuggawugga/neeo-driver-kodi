		...
		return new BluePromise((resolve, reject) => {
			...
			this.library.getSongDetails(song_id).then((song) => {
				...
				var item_out = {
					...
					browseIdentifier: '/music/album/' + song.albumid,
				};
				if(song.thumbnail) {
				...
				list_out.addListButtons([
					{ title: 'Play Song', actionIdentifier: 'Internal/playSong/' + song_id },
					{ title: 'Queue Song', actionIdentifier: 'Internal/queueSong/' + song_id }
				]);
				...
				if(song.albumid) {
					...
					list_out.addListButtons([
						{ title: 'Play Album', actionIdentifier: 'Internal/playAlbum/' + album_id + '/' + track },
						{ title: 'Queue Album', actionIdentifier: 'Internal/queueAlbum/' + album_id }
					]);
