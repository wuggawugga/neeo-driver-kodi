	...
	this.list = function(route_in, params_in) {
		return new BluePromise((resolve, reject) => {
			...
			this.browser.client.send('VideoLibrary.getMusicVideoDetails', params_out).then((response) => {
				...
				let list_buttons = [
					{title: 'Play', actionIdentifier: [ 'VideoPlayer', 'playMusicVideo', music_video_id ].join('|'), uiAction: 'close' },
					{title: 'Queue', actionIdentifier: [ 'VideoPlayer', 'queueMusicVideo', music_video_id ].join('|'), uiAction: 'close' },
				];
				list_out.addListButtons(list_buttons);
				list_buttons = [
					{ iconName: 'Repeat', actionIdentifier: [ 'VideoPlayer', 'cycleRepeat' ].join('|') },
					{ iconName: 'Shuffle', actionIdentifier: [ 'VideoPlayer', 'toggleShuffle' ].join('|') }
				];
				list_out.addListButtons(list_buttons);
