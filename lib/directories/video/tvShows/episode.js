			...
			this.library.getEpisodeDetails(episode_id).then((episode) => {
				...
				this.library.getSeasonDetails(season_id).then((season) => {
					this.library.getTvShowDetails(tvshow_id).then((tvshow) => {
						...
						if(episode.rating) item_out.label += ' ' + Number.parseFloat(episode.rating).toPrecision(2);
						...
						let list_buttons = [
							{title: 'Play', actionIdentifier: [ 'Internal', 'playEpisode', episode_id ].join('|'), uiAction: 'close' },
							{title: 'Queue', actionIdentifier: [ 'Internal', 'queueEpisode', episode_id ].join('|'), uiAction: 'close' },
