		...
		return new BluePromise((resolve, reject) => {
			...
			this.library.getTvShowDetails(tvshow_id).then((tvshow) => {
				this.library.getSeasons(tvshow_id).then((seasons) => {
					...
					let list_buttons = [];
//					if(tvshow.runtime) list_buttons.push({title: String.fromCodePoint(8987) + '' + Math.floor(tvshow.runtime/60) + 'm', inverse: true});
					if(tvshow.rating) list_buttons.push({title: String.fromCodePoint(11088) + '' + Number.parseFloat(tvshow.rating).toPrecision(2) + ' rating', inverse: true});
