...
function beginDiscovery() {
	...
	mdns_browser_http = bonjour.find({ type: 'xbmc-jsonrpc-h' }, function (data) {
		...
		probe_timeout_id = setTimeout(() => {probeDevice(service)}, 3000);
	});
	// WS API
			...
			instance.post(uri, request_config)
				.then(function (response) {
					if(response.status == 200) {
						debug('API unauthenticated');
						conf.set('kodi_instances.' + id + '.auth', false);
						conf.set('kodi_instances.' + id + '.registered', true);
					}
				})
				.catch(function (error) {
					if(error.response && error.response.status == 401) {
						debug('API needs auth');
		...
		if(!service.mac) {
			...
			ws.on('error', function foo(error) {
				if(error.code == 'ECONNREFUSED') {
					...
					conf.set('kodi_instances.' + service.id + '.remote_control_settings', false);
